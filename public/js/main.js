...
Array.from(item).forEach((element) => {
  element.addEventListener("dblclick", markComplete); //Adding an event listener to each of the spans
});

Array.from(itemCompleted).forEach((element) => {
  element.addEventListener("dblclick", markUnComplete); //Adding an event listener to each span with a completed
});
...
async function markComplete() {
  ...
  try {
    const response = await fetch("markComplete", {
      //Submitting the PUT request to the server
      method: "put",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        itemFromJS: itemText,
